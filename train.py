...
def train(args, model, device, train_loader, optimizer, criterion, epoch):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        target = [torch.Tensor(t).float().to(device) for t in target]
...
def main():
    ...
    if not args.resume:
        ...
        model.face_head.apply(weights_init)
        model.head_head.apply(weights_init)
        model.body_head.apply(weights_init)
